...
f(1) = -Dsb*(S-s)*beta/delta/mc + muo/Yo*s*o/(Kso+s)/(Ko+o) + ...
    mun/Yn*s*n/(Ksn+s)/(Kn+n)/(1+o/Kc);
%f(2) = -Dob*(O-o)*beta/delta/mc + gamma*muo*s*o*a/(Kso+s)/(Ko+o)/(Kao+a) + ...
%    alphao*ko*o/(Kop+o);
f(2) = -Dob*(O-o)*beta/delta/mc + gamma*muo*s*o/(Kso+s)/(Ko+o) + ...
    alphao*ko*o/(Kop+o);
%f(3) = -Dnb*(N-n)*beta/delta/mc + (eta*mun*s*n*a/(Ksn+s)/(Kn+n)/(Kan+a) + ...
%    alphan*kn*n/(Knp+n))/(1+o/Kc);
f(3) = -Dnb*(N-n)*beta/delta/mc + (eta*mun*s*n/(Ksn+s)/(Kn+n) + ...
    alphan*kn*n/(Knp+n))/(1+o/Kc);
%f(4) = -Dab*(A-a)*beta/delta/mc + psi*muo*s*o*a/(Kso+s)/(Ko+o)/(Kao+a) + ...
%    epsilon*mun*s*n*a/(Ksn+s)/(Kn+n)/(Kan+a)/(1+o/Kc);
f(4) = -Dab*(A-a)*beta/delta/mc + psi*muo*s*o/(Kso+s)/(Ko+o) + ...
    epsilon*mun*s*n/(Ksn+s)/(Kn+n)/(1+o/Kc);
